...
@app.route('/fingerprint/list', methods=['GET'])
def list_fingerprints():
    try:
        if finger.read_templates() == adafruit_fingerprint.OK:
            return jsonify({"status": "success", "templates": finger.templates})
        else:
            return jsonify({"status": "failure", "message": "Failed to read templates"})
